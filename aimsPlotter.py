...
fname = '/home/foxcatcher/Desktop/PPSTM/geometry.in'  # The name of (and the path to) your file
...
def loadGeometryIn(fname):
    ...
    for i in np.arange(10000):
        ...
        if len(ws) > 0:
            if ws[0] == 'atom':
                e.append(ws[4])
                xs.append(float(ws[1]))
                ys.append(float(ws[2]))
                zs.append(float(ws[3]))
                q.append(0)
    ...
    f.close()
    print("lvec = ", lvec)
    print("e,x,y,z = ", e, xs, ys, zs)
    return e, xs, ys, zs, lvec
...
e, xs, ys, zs, lvec = loadGeometryIn(fname)
...
plt.yticks([])
cols = pltcolor(e)
plt.scatter(xs, ys, zorder=1, marker='o', alpha=1.0, facecolors=cols, edgecolors=cols)
